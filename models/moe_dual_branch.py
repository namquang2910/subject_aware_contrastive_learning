"""
Mixture-of-Experts Dual-Branch Encoder for Subject-Aware SSL.
...
------------
                     Input
                       │
                 ┌─────▼──────┐
                 │ Shared Stem │  (2 CNN blocks – cheap feature extractor)
                 └─────┬──────┘
                       │ stem features  [B, C_stem, L']
                       │
              ┌────────▼────────┐
              │  Router Network  │  Linear(flat_stem → 2) → Softmax(τ)
              │  g = [g_inv,     │  → soft gating weights  [B, 2]
              │       g_spec]    │
              └──┬───────────┬──┘
                 │           │
          g_inv  │           │  g_spec
         ...
         │ Invariant │   │ Subject-  │
         │ Branch    │   │ Specific  │
         │ CNN+Linear│   │ CNN+Linear│
         └───────┬──┘   └────┬──────┘
                 │           │
          h_inv [B,D]  h_spec [B,D]
                 │           │
                 └─────┬─────┘
                       │  h = g_inv·h_inv + g_spec·h_spec   [B, D]
                       ▼
              Shared output embedding h

PRETRAINING losses (all optional via config):
  1. L_inv    = NCE(proj_inv(h_inv), view2)          – cross-subject invariance
  2. L_spec   = NCE(proj_spec(h_spec), same-subject) – subject-specific consistency
  3. L_subj   = CE(subj_head(h_spec), subject_id)    – explicit subject classification
  4. L_adv    = CE(subj_head_adv(GRL(h_inv)),        – adversarial: push h_inv away
                   subject_id)                          from subject identity
  5. L_orth   = ||h_inv^T · h_spec||_F / B            – orthogonality regulariser

FINETUNING:
  h_cat = concat[h_inv, h_spec]   [B, 2·D]
  → Linear(2D → num_class) → BCE
"""

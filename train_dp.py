...
class Trainer:
    def __init__(self, cfg, logger, num_class, train_loader, train_sampler, device, rank=0, world_size=1):
        ...
        if model_type == "contrastive":
            self.model = contrastive_model.ContrastiveModel(
            ...
            self.model = subject_invariant_model.AdversarialContrastiveModel(
                encoder,
            ...
            self.model = subject_specific_model.SubjectSpecificContrastiveModel(
                encoder, projection_output=projection_output,
                device=self.device
            )
    ...
    def train_one_epoch(self, epoch):
        ...
        if self.rank == 0 and epoch % self.print_freq == 0:
            ...
            self.logger.info(f"[Epoch {epoch:03d}] {loss_str}, lr={lr:.6f}")
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            out = self.train_one_epoch(epoch)

            # Early stopping only on rank 0
            if self.rank == 0:
                should_stop, improved = self.early_stopper.step(out['total_loss'])
            else:
...
#                    self.save_checkpoint(ckpt_path)
                ...
                if improved:
                    self.best_total_loss = out['total_loss']

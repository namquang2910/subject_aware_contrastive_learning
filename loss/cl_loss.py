...
class NCELoss(nn.Module):
    ...
    """
    def __init__(self, temperature=0.1, maximize_loss = False, positive_dif=False):
        super().__init__()
        self.temperature = temperature
        self.maximize_loss = maximize_loss
        self.positive_dif = positive_dif
    ...
    def forward(self, embeddings_v1, embeddings_v2, key_ids=None):
        ...
        # For subject-invariant: use NEGATIVE pairs instead of positive
        if self.positive_dif:
            pos_mask = key_ids1 != key_ids2  # Different subjects = positive pairs
        else:
            pos_mask = key_ids1 == key_ids2  # Same subjects = positive pairs

...
##################################################################################
## Note: the Composer was at the end                                             #
########################### List data augumnetation ##############################
##### sample data include: ['x', 'x_base', 'x_left_buffer', 'x_right_buffer'] ####
##################################################################################
...
class LowPassFilterDeterministic:
    def __init__(self, data_freq=128, highcut_hz=0.05):
        """
...
class LowPassFilterStochastic:
    def __init__(self, data_freq=128, highcut_hz_min=0.01, highcut_hz_max=1, n_steps=1000):
        """
        Apply low pass filter to remove frequency bands >= highcut_hz
        :param data_freq: frequency of data to apply filter to (e.g., 4Hz for EDA)
...
class HighPassFilterDeterministic:
    def __init__(self, data_freq=128, lowcut_hz=0.05):
        """
...
class HighPassFilterStochastic:
    def __init__(self, data_freq=128, lowcut_hz_min=0.05, lowcut_hz_max=0.25, n_steps=1000):
        """
        Apply high pass filter to remove frequency bands <= lowcut_hz
        :param data_freq: frequency of data to apply filter to (e.g., 4Hz for EDA)
        :param lowcut_hz: upper bound on frequency bands to remove
...
class BandstopFilterDeterministic:
    def __init__(self, data_freq=128, remove_freq=0.25, Q=0.707):
        """
...
class BandstopFilterStochastic:
    def __init__(self, data_freq=128, remove_freq_min=0.01, remove_freq_max=1.0, Q=0.707):
        """
...
class BandpassFilterDeterministic:
    def __init__(self, data_freq=128, keep_freq=0.25, Q=0.707):
        """
...
@lru_cache(maxsize=None)
def memoize_scipy_bandpass_butter_design_ba(order, lowcut_hz, highcut_hz, fs):
    """
    Memoized Butterworth band-pass filter design.
    """
...
class BandpassFilterStochastic:
    def __init__(
        ...
        """
        Apply a stochastic band-pass filter.

        A random pair (lowcut_hz, highcut_hz) is sampled each call:
          lowcut_hz  ~ [lowcut_hz_min,  lowcut_hz_max]
          highcut_hz ~ [highcut_hz_min, highcut_hz_max]

        :param data_freq: sampling frequency of data (e.g., 250 Hz for ECG)
        :param lowcut_hz_min: minimum lower cutoff frequency
        :param lowcut_hz_max: maximum lower cutoff frequency
        :param highcut_hz_min: minimum upper cutoff frequency
        :param highcut_hz_max: maximum upper cutoff frequency
        :param n_steps: number of discretized cutoffs to sample from
    ...
    def _sample_band(self):
        fs_nyq = self.data_freq / 2.0

        low_grid = np.linspace(self.lowcut_hz_min, self.lowcut_hz_max, self.n_steps)
        high_grid = np.linspace(self.highcut_hz_min, self.highcut_hz_max, self.n_steps)

        for _ in range(10):  # try a few times to get a valid band
            lowcut_hz = np.random.choice(low_grid)
            highcut_hz = np.random.choice(high_grid)

            if 0 < lowcut_hz < highcut_hz < fs_nyq:
                return lowcut_hz, highcut_hz

        # fallback: use midpoints if random sampling fails
        lowcut_hz = 0.5 * (self.lowcut_hz_min + self.lowcut_hz_max)
        highcut_hz = 0.5 * (self.highcut_hz_min + self.highcut_hz_max)
        highcut_hz = min(highcut_hz, fs_nyq * 0.99)
        if not (0 < lowcut_hz < highcut_hz < fs_nyq):
            raise ValueError(
                f"Cannot form valid bandpass with given ranges for fs={self.data_freq}"
            )
        return lowcut_hz, highcut_hz
    ...
    def __call__(self, sample_data):
        ...
        lowcut_hz, highcut_hz = self._sample_band()

        b, a = memoize_scipy_bandpass_butter_design_ba(
            order=self.order,
            lowcut_hz=lowcut_hz,
...
class TonicConstantAmplitudeScalingDeterministic:
    ...
    """
    def __init__(self, tonic_scale_factor, data_freq=128):
...
class TonicConstantAmplitudeScalingStochastic:
    ...
    Skin pharmacology and physiology, 31(6), 298-307.
    """
    def __init__(self, tonic_scale_factor_min=0.5, tonic_scale_factor_max=1.5, data_freq=128):
...
class TonicAmplitudeWarpingDeterministic:
    ...
    Skin pharmacology and physiology, 31(6), 298-307.
    """
    def __init__(self, sigma=0.1, knot=4, data_freq=128):
        self.sigma = sigma
        self.knot = knot
        # design the filters to extract tonic and phasic
        self.data_freq = data_freq
...
class TonicAmplitudeWarpingStochastic:
    ...
    Skin pharmacology and physiology, 31(6), 298-307.
    """
    def __init__(self, sigma_min=0.01, sigma_max=0.25, knot_min=0, knot_max=4, data_freq=128):
        self.sigma_min = sigma_min
        self.sigma_max = sigma_max
        self.knot_min = knot_min
        self.knot_max = knot_max
...
class FlipWrist:
    def __call__(self, sample_data):
        x_opp_wrist = sample_data['x_opp_wrist']
        return x_opp_wrist
...
TRANSFORM_REGISTRY = {
    # === Gaussian Noise ===
    "GaussianNoiseDeterministic": GaussianNoiseDeterministic,
    "GaussianNoiseStochastic": GaussianNoiseStochastic,

    # === Filters ===
    "LowPassFilterDeterministic": LowPassFilterDeterministic,
    "LowPassFilterStochastic": LowPassFilterStochastic,
    "HighPassFilterDeterministic": HighPassFilterDeterministic,
    "HighPassFilterStochastic": HighPassFilterStochastic,
    "BandpassFilterDeterministic": BandpassFilterDeterministic,
    "BandpassFilterStochastic": BandpassFilterStochastic,
    "BandstopFilterDeterministic": BandstopFilterDeterministic,
    "BandstopFilterStochastic": BandstopFilterStochastic,

    # === Frequency Noise ===
    "HighFrequencyNoiseDeterministic": HighFrequencyNoiseDeterministic,
    "HighFrequencyNoiseStochastic": HighFrequencyNoiseStochastic,

    # === Temporal transforms ===
    "TemporalCutoutDeterministic": TemporalCutoutDeterministic,
    "TemporalCutoutStochastic": TemporalCutoutStochastic,
    "PermuteDeterministic": PermuteDeterministic,
    "PermuteStochastic": PermuteStochastic,
    "TimeShiftDeterministic": TimeShiftDeterministic,
    "TimeShiftStochastic": TimeShiftStochastic,
    "TimeWarpingDeterministic": TimeWarpingDeterministic,
    "TimeWarpingStochastic": TimeWarpingStochastic,

    # === Amplitude transforms ===
    "AmplitudeWarpingDeterministic": AmplitudeWarpingDeterministic,
    "AmplitudeWarpingStochastic": AmplitudeWarpingStochastic,
    "ConstantAmplitudeScalingDeterministic": ConstantAmplitudeScalingDeterministic,
    "ConstantAmplitudeScalingStochastic": ConstantAmplitudeScalingStochastic,

    # === EDA-specific / Component extraction ===
    "ExtractPhasic": ExtractPhasic,
    "ExtractTonic": ExtractTonic,
    "TonicConstantAmplitudeScalingDeterministic": TonicConstantAmplitudeScalingDeterministic,
    "TonicConstantAmplitudeScalingStochastic": TonicConstantAmplitudeScalingStochastic,
    "TonicAmplitudeWarpingDeterministic": TonicAmplitudeWarpingDeterministic,
    "TonicAmplitudeWarpingStochastic": TonicAmplitudeWarpingStochastic,

    # === Artifacts ===
    "JumpArtifactDeterministic": JumpArtifactDeterministic,
    "JumpArtifactStochastic": JumpArtifactStochastic,
    "LooseSensorArtifactDeterministic": LooseSensorArtifactDeterministic,
    "LooseSensorArtifactStochastic": LooseSensorArtifactStochastic,
    ...
    "FlipWrist": FlipWrist,
    "ScalePhasic": ScalePhasic,
    "DCShift": DCShift,
}


class Composer:
    def __init__(self, transform_dict, global_args):
        """
        ...
        """
        self.transform_dict = self.get_transforms(transform_dict)
        self.n_transforms = global_args.get("n_transforms", 1)
        self.stochastic_choice = global_args.get("stochastic_choice", True)
        print(self.n_transforms)
        print(self.transform_dict)
        
    def get_transforms(self, transform_dict):
        transforms = []
        for trf_name, trf_args in transform_dict.items():
            trf_class = TRANSFORM_REGISTRY.get(trf_name)
            if trf_class is None:
                valid = ", ".join(sorted(TRANSFORM_REGISTRY.keys()))
                print(trf_args)
                raise ValueError(f"Unrecognized transform '{trf_name}'. Valid options: {valid}")
            transforms.append(trf_class(**(trf_args or {})))
        if not transforms:
            raise ValueError("No transforms constructed. Check your config.")
        return transforms
    ...
    def __call__(self, sample_data):
        ...
        for i in range(self.n_transforms):
            trf = np.random.choice(self.transform_dict) if self.stochastic_choice else self.transform_dict[i]
            out['x'] = trf(out).copy()
        return out
